...
sudo apt-get install seclists gobuster ghidra fping nmap ntp crackmapexec htop grep sed python3-impacket john responder -y

# Development stuff
sudo apt-get install vim git cmake g++ build-essential curl net-tools neofetch htop wget nmap ranger -y

# Graphical stuff
sudo apt-get install kitty linux-headers-generic baobab libuv1-dev flameshot shotwell mupdf -y
